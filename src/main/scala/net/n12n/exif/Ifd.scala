...
abstract class Ifd(exif: ExifSegment, offset: Int, val name: String) {
  ...
  /** Offset to next IDF relative to the Exif's TiffHeader.
    */
  lazy val nextIfd = exif.data.toSignedLong(exif.tiffOffset + offset + 2 + count * IfdAttribute.Length, exif.byteOrder)
  ...
  def findValue[V](tag: TypedTag[V]): Option[V] = attributes.find(_.tag.equals(tag)).map(tag.value(_, exif.byteOrder))
  ...
  def value[V](tag: TypedTag[V]): V = {
    val ifdAttr: IfdAttribute = attributes.find(_.tag == tag).getOrElse(throw AttributeNotFoundException(tag.name))
